        ...
        <div class="row">
            <div class="col-xs-12 col-md-9 write-wrap">
                ...
                <div id="replyList"> 
                    ...
                    자바스크립트 단에서 반복문을 이용해서 댓글의 개수만큼 반복표현
...
<script>
    window.onload = function () {
        ...
        document.getElementById('moreList').onclick=()=>{
            /*
            왜 false 인가
            더보기 이기 때문에  누적해야함계속해서
            */
            getList(++page, false);
        }

        let page = 1; //전역의미로 사용할 페이지 번호
        let strAdd = ''; // 화면에 그려넣을 태그를 문자열의 형태로 추가할 변수
        const $replyList = document.getElementById('replyList');

        getList(1,true);//게시글 상세보기 화면에 처음 진입했을 시 댓글리스트를 한번 불러오자
        ...
        function getList(pageNum, reset) {
            ...
            fetch('${pageContext.request.contextPath}/reply/getList/' + bno + '/' + pageNum)
                ...
                .then(data => {
                    ...
                    //댓글 내용 태그를 ㅜ적하고 있는 strAdd 변수를 초기화 해서
                    //마치 화면이 리겟된 것처럼 보여줘야합니다.
                    ...
                    //페이지번호 * 이번 요청으로 받은 대슬수보다 전체 댓글 개수가 작다면 더보기 버튼은 없어도 된다.
                    ...
                    } else document.getElementById('replyList').insertAdjacentHTML('afterbegin',strAdd);
                });
        } //end getList();
		...
		그러다 보니 이벤트가 등록되는 시점보다 fetch 함수의 실행이 먼저 끄날 것이라는
		보장이 없ㄱ ㅣ댸문에 해당 방식은 이벤트 등록이 불가능합니다.
		이 때는, 이미 실제로 존재하는 #replyList에 등록하고, 이벤트를 자식에게 위임하여
		사용하는 addEventListener 를 통해 처리를 해야 합니다.
		*/
		...
		document.getElementById('modalDelBtn').onclick = () => {
			...
			삭제를 진행하고
			4. 만약 비밀번호가 틀렸다면 문자열을 반환해서 '비밀번호가 틀렸습니다.' 경고창 ㄱㄱ
			
			삭제가 완료되면 모달 닫고 목록 요청 다시 보내세요. (reset의 여부를 잘 판단)
			*/
